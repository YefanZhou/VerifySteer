...
if envs.VLLM_USE_PRECOMPILED:
    assert _is_cuda(), "VLLM_USE_PRECOMPILED is only supported for CUDA builds"
    _existing_so = list((ROOT_DIR / "vllm").glob("**/*.abi3.so"))
    if not _existing_so:
        raise RuntimeError(
            "VLLM_USE_PRECOMPILED=1 set but no .so files found under vllm/. "
            "Run `python download_so.py` first."
        )
    print(f"[precompiled] Found {len(_existing_so)} .so files, skipping wheel download.")
    package_data.setdefault("vllm", []).extend(
        [str(f.relative_to(ROOT_DIR / "vllm")) for f in _existing_so]
    )
