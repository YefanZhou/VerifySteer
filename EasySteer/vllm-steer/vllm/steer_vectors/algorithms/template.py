...
class AlgorithmTemplate(BaseSteerVectorAlgorithm, ABC):
    ...
    def set_steer_vector(self, index: int, **kwargs) -> None:
        ...
        if isinstance(payload, torch.Tensor):
            print('!!! >>>> For Tensor payload: apply scale_factor directly')
            payload = payload * scale_factor
        elif isinstance(payload, dict):
            print(f'!!! >>>> For dict payload: add scale_factor {scale_factor:.3f} to the dict')
            payload = {**payload, "scale_factor": scale_factor}
            if "beta" in kwargs:
                print(f'!!! >>>> For dict payload: add beta {kwargs["beta"]:.3f} to the dict')
                payload["beta"] = kwargs["beta"]
            if "kappa" in kwargs:
                print(f'!!! >>>> For dict payload: add kappa {kwargs["kappa"]:.3f} to the dict')
                payload["kappa"] = kwargs["kappa"]
            if "alpha" in kwargs:
                print(f'!!! >>>> For dict payload: add alpha {kwargs["alpha"]:.3f} to the dict')
