...
@register_algorithm("gated_norm_direct_cosine_mask")
class GatedNormalizeDirectAlgorithmCosineMask(AlgorithmTemplate):
    ...
    def _transform(self, hidden_state: torch.Tensor, params: dict) -> torch.Tensor:
        ...
        beta = params['beta']
        h_norm = hidden_state / hidden_state.norm(dim=-1, keepdim=True)
        v_norm = vectors / vectors.norm()
        v_norm = v_norm.to(hidden_state.dtype)
        
        awareness = (h_norm @ v_norm < beta).unsqueeze(-1) #.float()
        
        norm_pre = torch.norm(hidden_state, dim=-1, keepdim=True)
        transformed = hidden_state + scale_factor * vectors
        norm_post = torch.norm(transformed, dim=-1, keepdim=True)
        steered_hidden = transformed * norm_pre / norm_post
        
        returned_hidden = torch.where(awareness, steered_hidden, hidden_state)
        
        return returned_hidden
    ...
    def load_from_path(cls, path: str, device: str, **kwargs) -> dict:
        ...
        if config is None:
            raise ValueError("GatedNormalizeDirectAlgorithmCosineMask.load_from_path requires 'config' in kwargs")
    ...
    def _load_from_gguf(cls, path: str, device: str, **kwargs) -> dict:
        ...
        #config.adapter_dtype = torch.bfloat16
        ...
        for tensor in reader.tensors:
            ...
            #                   torch.from_numpy(np_copy).to(device).to(torch.bfloat16)}
            sv_weights[layer] = {'vector': 
